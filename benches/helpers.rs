...
pub(crate) const SYNC_METHODS: [&str; 3] =
    [SYNC_FAST_CALL, SYNC_MEM_CALL, SYNC_SLOW_CALL];
pub(crate) const ASYNC_METHODS: [&str; 3] =
    [SYNC_FAST_CALL, SYNC_MEM_CALL, SYNC_SLOW_CALL];
...
pub async fn http_server(
...
) -> (String, jsonrpc_http_server::Server) {
    ...
    io.add_method(ASYNC_FAST_CALL, |_| {
        async { Ok(Value::String("lo".to_string())) }
    });
    io.add_sync_method(SYNC_MEM_CALL, |_| {
        Ok(Value::String("A".repeat(1 * 1024 * 1024)))
    });
    io.add_method(ASYNC_MEM_CALL, |_| {
        async { Ok(Value::String("A".repeat(1 * 1024 * 1024))) }
    });
    io.add_sync_method(SYNC_SLOW_CALL, |_| {
        std::thread::sleep(std::time::Duration::from_millis(1));
        Ok(Value::String("slow call".to_string()))
    });
    io.add_method(ASYNC_SLOW_CALL, |_| {
        async {
...
pub async fn ws_server(
    handle: tokio::runtime::Handle,
) -> (String, jsonrpc_ws_server::Server) {
    use std::sync::atomic::{
        AtomicU64,
        Ordering,
    };
    ...
    io.add_subscription(
        ...
        (UNSUB_METHOD_NAME, |_id: SubscriptionId, _| {
...
pub async fn http_server(
    handle: tokio::runtime::Handle,
) -> (String, jsonrpsee::http_server::HttpServerHandle) {
...
pub async fn ws_server(
    handle: tokio::runtime::Handle,
) -> (String, jsonrpsee::ws_server::WsServerHandle) {
...
fn gen_rpc_module() -> jsonrpsee::RpcModule<()> {
    ...
    module
        ...
        .unwrap();
    module
        .register_async_method(ASYNC_FAST_CALL, |_, _| async { Ok("lo") })
        .unwrap();

    module
        .register_method(SYNC_MEM_CALL, |_, _| Ok("A".repeat(1 * 1024 * 1024)))
        .unwrap();

    module
        .register_async_method(ASYNC_MEM_CALL, |_, _| {
            async move { Ok("A".repeat(1 * 1024 * 1024)) }
        })
        .unwrap();
    ...
    module
        .register_async_method(ASYNC_SLOW_CALL, |_, _| {
            async move {
                tokio::time::sleep(std::time::Duration::from_millis(1)).await;
                Ok("slow call async")
            }
