...
impl HttpTransportClient {
    ...
    pub(crate) fn new(
        ...
        let target: Uri = target
            .as_ref()
            .parse()
            .map_err(|e| Error::Url(format!("Invalid URL: {}", e)))?;
    ...
    async fn inner_send(
        ...
        let req = hyper::Request::post(&self.target)
            .header(
                ...
                hyper::header::HeaderValue::from_static(CONTENT_TYPE_JSON),
            )
            .header(
                hyper::header::ACCEPT,
                hyper::header::HeaderValue::from_static(CONTENT_TYPE_JSON),
...
mod tests {
    ...
    fn assert_target(
        ...
        assert_eq!(
            client.target.path_and_query().map(|pq| pq.as_str()),
            Some(path_and_query)
        );
    ...
    fn invalid_http_url_rejected() {
        let err =
            HttpTransportClient::new("ws://localhost:9933", 80, CertificateStore::Native)
                .unwrap_err();
